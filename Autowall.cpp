...
bool SimulateFireBullet(IClientEntity* entity, IClientEntity *local, CBaseCombatWeapon *weapon, FireBulletData &data)
{
	...
	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f))
	{
		...
		if ((data.enter_trace.hitgroup <= 7) && (data.enter_trace.hitgroup > 0) && pClass->m_ClassID != (int)CSGOClassID::CHostage && (local->GetTeamNum() != data.enter_trace.m_pEnt->GetTeamNum() || Menu::Window.RageBotTab.TargetFriendlyFire.GetState()))
		{
